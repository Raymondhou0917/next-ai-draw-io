...
export default function ChatPanel({
    ...
    return (
        ...
        >
            ...
            >
                {/* AI 生成狀態提示 - 顯示在額度上方 */}
                {/* 在用戶發送訊息後（submitted）、AI 回覆中（streaming）、或圖表生成中都顯示 */}
                {(status === "submitted" || status === "streaming" || diagramGeneratingState !== "idle") && (
                    <div className="flex items-center gap-2 px-3 py-2 mb-2 rounded-lg bg-primary/10 border border-primary/20 animate-pulse">
                        ...
                        <span className="text-xs font-medium text-primary">
                            {/* 根據狀態顯示不同文字 */}
                            {diagramGeneratingState === "rendering"
                                ? (dict.diagramLoading?.rendering || "正在渲染圖表...")
                                : diagramGeneratingState === "generating"
                                    ? (dict.diagramLoading?.generating || "正在生成圖表...")
                                    : status === "submitted"
                                        ? (dict.diagramLoading?.thinking || "AI 正在思考...")
                                        : (dict.diagramLoading?.responding || "AI 正在回覆...")}
                        </span>
                    </div>
                )}
                ...
                <ChatInput
                    ...
                    onConfigureClick={() => setShowModelConfigDialog(true)}
